...
import { structToProtoApi } from '#/api/system/tool';
import { $t } from '#/locales';

defineOptions({ name: 'DevtoolStructToProto' });

const inputStr = ref('');
const outputStr = ref('');
const loading = ref(false);

async function handleStructToProto() {
  ...
  loading.value = true;
  try {
    const result = await structToProtoApi({ structStr: inputStr.value });
    outputStr.value = result.protoStr ?? '';
    if (outputStr.value) {
...
<template>
  <Page auto-content-height>
    <div class="flex flex-col gap-4 p-6">
      <a-card :title="$t('devtool.struct.toProto.title')">
        ...
        <div class="mt-4 flex gap-3">
          <a-button type="primary" :loading="loading" @click="handleStructToProto">
            {{ $t('devtool.struct.toProto.action') }}
          </a-button>
