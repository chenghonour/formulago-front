...
function transformMenu(
  ...
  const meta: RouteMeta = {
    ...
    keepAlive: backendMenu.meta
      ? !backendMenu.meta.ignoreKeepAlive
      : undefined,
    affixTab: backendMenu.meta?.affix || undefined,
    iframeSrc: backendMenu.meta?.frameSrc || undefined,
    hideChildrenInMenu:
      backendMenu.meta?.hideChildrenInMenu || undefined,
  };
...
export async function getAllMenusApi() {
  const result = await requestClient.get<BackendMenuInfo[]>(
    '/admin/menu/role',
  );
  // 兼容分页响应格式：{ total, data, errCode } 返回完整对象，需提取 data
  const menuData = Array.isArray(result) ? result : (result as Record<string, any>)?.data ?? [];
  let routes = (menuData as BackendMenuInfo[])
  ...
  if (routes.length === 1) {
    ...
    if (first && (first.name === 'root' || (!first.path && !!first.children?.length))) {
