...
import { SearchX, X } from '@formulago/icons';
import { $t } from '@formulago/locales';
import { preferences } from '@formulago/preferences';
import { traverseTreeValues, uniqueByField } from '@formulago/utils';
...
function search(searchKey: string) {
  ...
  traverseTreeValues(searchItems.value, (item) => {
    // 同时匹配中文名 name 和英文名 nameEn
    if (
      reg.test(item.name?.toLowerCase()) ||
      (item.nameEn && reg.test(item.nameEn?.toLowerCase()))
    ) {
      results.push(item);
    }
  });
    ...
    <div class="flex! h-full justify-center px-2 sm:max-h-112.5">
      ...
      <ul v-show="searchResults.length > 0" class="w-full">
        ...
        >
          ...
          <span class="flex-1">{{ preferences.app.locale === 'en-US' && item.nameEn ? item.nameEn : item.name }}</span>
          <div
