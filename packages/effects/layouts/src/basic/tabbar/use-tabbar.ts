...
import type { TabDefinition } from '@formulago/types';
...
import { useContentMaximize, useTabs } from '@formulago/hooks';
import {
  ArrowLeftToLine,
  ArrowRightLeft,
  ArrowRightToLine,
  ...
  RotateCw,
  X,
} from '@formulago/icons';
import { $t, useI18n } from '@formulago/locales';
import { getTabKey, useAccessStore, useTabbarStore } from '@formulago/stores';
import { filterTree } from '@formulago/utils';
...
export function useTabbar() {
  ...
  function wrapperTabLocale(tab: RouteLocationNormalizedGeneric) {
    const title =
      locale.value === 'en-US' && tab?.meta?.titleEn
        ? (tab.meta.titleEn as string)
