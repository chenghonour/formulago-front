import type { MenuRecordRaw } from '@formulago/types';

import { computed, onBeforeMount, ref, watch } from 'vue';
import { useRoute } from 'vue-router';

import { preferences, usePreferences } from '@formulago/preferences';
import { useAccessStore } from '@formulago/stores';
import { findRootMenuByPath } from '@formulago/utils';
 ...
 * 根据当前语言深度转换菜单名
 */
function localizeMenus(menus: MenuRecordRaw[]): MenuRecordRaw[] {
  const isEn = preferences.app.locale === 'en-US';
  return menus.map((menu) => ({
    ...menu,
    name: isEn && menu.nameEn ? menu.nameEn : menu.name,
    children: menu.children ? localizeMenus(menu.children) : undefined,
  }));
}
...
function useMixedMenu() {
  ...
  const menus = computed(() => localizeMenus(accessStore.accessMenus));
