...
import { usePriorityValues } from '@formulago/hooks';
import { EmptyIcon } from '@formulago/icons';
import { $t } from '@formulago/locales';
import { usePreferences } from '@formulago/preferences';
import {
  cloneDeep,
  cn,
  isBoolean,
...
const options = computed(() => {
  ...
  if (mergedOptions.pagerConfig?.enabled !== false && mergedOptions.proxyConfig?.ajax) {
    const mobileLayouts = [
      'PrevJump',
      'PrevPage',
      'Number',
...
async function init() {
  ...
  // 先设置 proxy wrapper，确保后续查询使用归一化参数
